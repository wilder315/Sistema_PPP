...
def obtener_ultimos_semestres():
    ...
    try:
        with conexion.cursor() as cursor:
            cursor.execute("SELECT * FROM semestre_academico ORDER BY nombre DESC LIMIT 6")
            column_names = [desc[0] for desc in cursor.description]
            rows = cursor.fetchall()
            for row in rows:
                semestre_dict = dict(zip(column_names, row))
...
def obtener_ultimos_semestres_activo():
    conexion = obtener_conexion()
    if not conexion:
        return {"error": "No se pudo establecer conexión con la base de datos."} 
    semestres = []
    try:
        with conexion.cursor() as cursor:
            cursor.execute("SELECT * FROM semestre_academico where estado = 'A' ORDER BY nombre DESC LIMIT 1")
            column_names = [desc[0] for desc in cursor.description]
            rows = cursor.fetchall()
            for row in rows:
                semestre_dict = dict(zip(column_names, row))
